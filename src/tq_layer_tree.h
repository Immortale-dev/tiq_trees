...
namespace Tiq::Tree {
	...
	class LayersCollection : public CountTree<LayerStatNode<K>, A> {
		public:
			...
			void merge(LayersCollection<K,A>& collection);
	...
	class LayerNode : public CountNode<T> {
		...
		public:
			using layer_key_t = typename L::key_t;
	...
	class LayerTree : public CountTree<N, A> {
		...
		public:
			...
			const node_ptr_t erase(const_node_ptr_t node);
			...
			const_node_ptr_t find_nth(size_t count, layer_ket_t layer) const;
