...
TEST_SRCS:=$(wildcard $(TEST_SRCPATH)/*.$(CPP_SRC_EXT)) $(EXTRA_TEST_SRCS)
...
TEST_OBJS=$(addprefix $(BUILD_DIR)/,$(TEST_SRCS:%.$(CPP_SRC_EXT)=%.o))
...
MOD_INC=$(addsuffix /$(SRCINCL),$(LIBS))

INCL=-I$(SRCINCL) -I. -I$(DEP_DIR) $(addprefix -I,$(LIBS_INC)) $(addprefix -I,$(MOD_INC)) $(EXT_INCL)
...
# Main test endpoint
test.exe: $(CURRENT_LIB) $(TEST_OBJS) $(TEST_O) $(LIBS_A) $(LIBS_SHARED) $(EXT_RULES)
	$(CC) -o test.exe $(TEST_OBJS) $(TEST_O) $(CURRENT_LIB) $(LFLAGS) $(LIBS_LD) $(LIBS_A) $(EXT_LIBS) -L$(SHARED_DIR) $(SHARED_INCL)

# Custom test endpoint
mtest.exe: $(CURRENT_LIB) $(MTEST_O) $(LIBS_A) $(LIBS_SHARED) $(EXT_RULES)
	$(CC) -o mtest.exe $(MTEST_O) $(CURRENT_LIB) $(LFLAGS) $(LIBS_LD) $(LIBS_A) $(EXT_LIBS) -L$(SHARED_DIR) $(SHARED_INCL)

# Program executable
main.exe: $(CURRENT_LIB) $(MAIN_O) $(LIBS_A) $(LIBS_SHARED) $(EXT_RULES)
	$(CC) -o main.exe $(MAIN_O) $(CURRENT_LIB) $(LFLAGS) $(LIBS_LD) $(LIBS_A) $(EXT_LIBS) -L$(SHARED_DIR) $(SHARED_INCL)
